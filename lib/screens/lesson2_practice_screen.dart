...
class _ScreenState extends State<Lesson2PracticeScreen>
  ...
  bool get _answered {
    ...
    if (_q is _QConnectPairs) return _activitySubmitted;
  ...
  bool _isAnswerCorrect() {
    ...
    if (_q is _QConnectPairs) return true; // Only triggers onAllMatched when 100% correct
  ...
  Widget _buildTypeBadge() {
    final (label, icon, color) = switch (_q.type) {
      _QType.connectTool => ('Connect the Tool', Icons.timeline_rounded, const Color(0xFF6A1B9A)),
      ...
      _QType.mainUseMatch => ('Main Use Match', Icons.link_rounded, const Color(0xFFC2185B)),
      _QType.workstationConnect => ('Tool Workstation', Icons.construction_rounded, const Color(0xFF4527A0)),
  ...
  Widget _buildActivity() {
    ...
    if (_q is _QConnectPairs) {
      final q = _q as _QConnectPairs;
      return Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          Text(
          ...
          const SizedBox(height: 16),
          _ConnectDotsActivity(
            pairs: q.pairs,
            onAllMatched: _onAllMatched,
          ),
        ],
      );
    }
  ...
  String _getExplanation() {
    ...
    if (_q is _QConnectPairs) return 'All connections are correct!';
